...
Enrichment of liabilities in the top K is tested against all other DE-eligible
perturbations with Fisher's exact test (binary flags) and the Mann-Whitney U test
(continuous disruption measures).
...
def build_scores(top_k: int) -> pd.DataFrame:
    ...
    frame = stim[[gene_col, "naive_suppression", "tolerance_suppression", "n_total_de_genes"]].copy()
    frame = frame.rename(columns={"n_total_de_genes": "stim_de_genes"})

    rest_de = rest.set_index(gene_col)["n_total_de_genes"]
    ...
    frame = frame.dropna(subset=["naive_suppression"]).copy()
...
def report(frame: pd.DataFrame, top_k: int) -> bool:
    ...
    print("\n--- binary liability enrichment (Fisher, one-sided greater) ---")
    for label, col in (("core-essential", "is_core_essential"), ("IEI (immunodeficiency)", "is_iei")):
        odds, pval, n_top, n_back = _fisher(frame[col], in_top)
        top_rate = n_top / max(int(in_top.sum()), 1)
        bg_rate = n_back / max(n_bg, 1)
        flag = pval < 0.05 and odds > 1
        verdicts.append(flag)
        print(
            f"  {label:24s} top {n_top:3d}/{int(in_top.sum()):<4d} ({top_rate:6.1%})  "
            f"bg {n_back:4d}/{n_bg:<5d} ({bg_rate:6.1%})  OR={odds:6.2f}  p={pval:.3g}"
            f"  {'ENRICHED' if flag else 'ns'}"
        )

    print("\n--- continuous disruption (Mann-Whitney U, one-sided greater) ---")
    for label, col in (
        ...
        bg_vals = frame.loc[~in_top, col].dropna()
        ...
        verdicts.append(flag)
        print(
            f"  {label:28s} top median {top_vals.median():9.2f}  "
    ...
    print("\n" + "=" * 72)
    if holds:
        print("VERDICT: the empirical bet HOLDS.")
        ...
        print("VERDICT: the empirical bet FAILS.")
        print("The naive ranking is no more toxic than background. The headline needs a rewrite.")
    print("=" * 72)
    return holds
