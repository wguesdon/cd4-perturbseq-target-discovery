...
abstract: |
  ...
  CRISPRi screen confirms do reduce IL-2, but which a safety gate refuses: 12 of the top 20 in a
  ...
  expression (module-level permutation p = 0.005, effective n = 1 module). We build a
  therapeutic-window score from what the screen measures, read viability directly off it, and
  recover approved immunomodulator targets above chance without being told about them
  (5 of the 20 assay-visible positives, 5.6x, permutation p = 0.0017; AUROC 0.542 spans chance).
  We then turn the same hostility on our own nomination rule. It recovers only 3 of the 6
  approved-drug targets the pipeline itself finds, and direction of effect can be adjudicated only
  for genes that a drug or a monogenic syndrome has already characterised, so it can demote a
  candidate but never promote one. Several asserted mechanisms were refuted in the course of this
  work, including two of our own and one the adversarial audit asserted against us. We report them
  all. The contribution is the decision layer and its audit, not a validated target.
format:
  html:
    theme:
      light: cosmo
