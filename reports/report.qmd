...
abstract: |
  ...
  it nominates targets that work and would leave a patient immunodeficient. Eighteen of the top
  ...
  production down. We then tested that headline number against itself, and most of it turned out
  to be bought by effect size: a shortlist matched on transcriptome-wide effect magnitude is
  rejected 15.3 times out of 20. What reversal ranking *specifically* brings in is collapse of the
  regulatory tolerance program, rejected 12 times in our top 20 against 4.0 in a magnitude-matched
  one. We build a therapeutic-window score with an evidence floor, a context selectivity
  requirement and a tolerance-preservation penalty, and we read viability directly off the screen.
  Without being told which drugs are well tolerated, the score sorts five approved immunomodulators
  into the three mechanism classes the clinic uses. Three mechanisms we asserted were refuted in the
  course of this work, and one the adversarial audit asserted against us was refuted in turn. We
  report all four.
format:
  html:
    theme:
...
def spec_row(quantity: str) -> pd.Series:
    ...
    return spec[spec["quantity"].str.startswith(quantity)].iloc[0]
...
HOM = spec_row("rejected on homeostasis")
...
because the data supports them rather than because a database exists, a test of whether each axis is
about the direction of a perturbation or merely its size, and an honest account of what the assay
cannot see.
...
3. **Tolerance preservation.** Perturbations in the top quartile of tolerance-module suppression
   are rejected. The tolerance module really is suppressed beyond its own null: against 200
   expression-matched random nine-gene modules, z = +3.87 in the naive top 100 versus +1.26 in
   background, p = 3.5e-13.
...
controlled the confound backwards. The confounder is transcriptome-wide effect magnitude, and the
...
Markdown(
    ...
    f"rejected** by the safety gate. The survivors are "
...
So we measured transcriptome-wide effect magnitude directly, as
...
Decomposed by axis, the picture is unambiguous and it is not the one we expected.
**Homeostasis**, which does most of the rejecting, rejects
`{python} f"{int(HOM['observed'])}"` of our top 20 and
`{python} f"{HOM['matched_mean']:.1f}"` of a magnitude-matched 20
(P = `{python} f"{HOM['p_matched_ge_observed']:.2f}"`). It is a magnitude readout.
**Tolerance** rejects `{python} f"{int(TOL['observed'])}"` of ours against
`{python} f"{TOL['matched_mean']:.1f}"` matched, P < 0.0001. It is the axis that is about the
*direction* of a perturbation rather than its *size*.
...
not survive test B at all. Tolerance is 0.07 correlated with magnitude, and perturbations that
*induce* the effector program *induce* tolerance rather than suppressing it.

Finally, rankings that never mention the effector program:

```{python}
r = rivals.copy()
r.columns = ["ranking", "median tolerance loss", "median z_L2", "rejected on tolerance", "rejected on homeostasis", "pass gate"]
r.round(3)
```
...
**What survives.** "Reversal is not enough" stands: a practitioner who ranks by reversal and acts
on the top 20 will act on 18 targets this gate refuses. But the *mechanism* we asserted, collateral
transcriptional damage, is largely a restatement of effect size. The mechanism that survives every
control is tolerance collapse. That is the claim, and it is the one the pre-registered primary
endpoint of `scripts/02_risk_kill_reversal.py` tests: one-sided van Elteren,
...
Markdown(
    ...
    + ". It rejects "
    + ", ".join(f"`{g}` for {r.replace('homeostasis', 'disrupting the resting transcriptome').replace('tolerance', 'collapsing tolerance')}"
...
0.72 rank-correlated with transcriptome-wide effect magnitude, and a magnitude-matched shortlist is
...
Our safety gate is not the one we planned, and neither is our explanation of why it works.
...
library denominator rather than the DE table, two colliders operate in opposite directions: only 217
of 682 library core-essentials ever reach the DE table, because their knockdown depletes cells and
fails differential-expression eligibility, and the `ontarget_significant` filter then removes the
nonessential controls instead, because it requires an expressed target, leaving 3 of 141. The
...
(P = `{python} f"{HOM['p_matched_ge_observed']:.2f}"`). The axis that survives every control is the
one built from a *graded module score* rather than a count: tolerance-module suppression, ρ = 0.07
with transcriptome-wide effect magnitude. The lesson is not "use the screen instead of a database".
...
transcriptome-wide effect magnitude. If it does not, the script exits non-zero and this report's
...
|score| decile, or on transcriptome-wide ‖z‖₂" as interchangeable controls; those two variables
